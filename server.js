...
app.get('/proxy', async (req, res) => {
    ...
    try {
        ...
        // --- RENDER-DEBUGGING ---
        console.log(`[RENDER-DEBUG] Incoming proxy request for: ${targetUrl}`);
        console.log(`[RENDER-DEBUG] Attempting proxy request to: http://${VM_PROXY_HOST}:${VM_PROXY_PORT}`);
        // --- END RENDER-DEBUGGING ---
        ...
        response = await axios.get(targetUrl, {
            ...
            maxRedirects: 0,
            // --- זוהי ההגדרה הקריטית שחסרה בקוד שלך! ---
            proxy: {
                host: VM_PROXY_HOST,
                port: VM_PROXY_PORT,
                protocol: 'http' // וודא ששרת הפרוקסי ב-VM מאזין ב-HTTP (ולא HTTPS)
            }
        });
        ...
        // --- RENDER-DEBUGGING: לוג שגיאות מפורט יותר ---
        console.error(`[RENDER-DEBUG] Proxy request failed for URL: ${targetUrl}`);
        console.error(`[RENDER-DEBUG] Error message: ${error.message}`);
        if (error.response) {
            console.error(`[RENDER-DEBUG] Target URL responded with status: ${error.response.status}`);
            console.error(`[RENDER-DEBUG] Target URL response data (partial): ${error.response.data ? error.response.data.toString('utf8').substring(0, 200) : 'N/A'}`);
            ...
            console.error(`[RENDER-DEBUG] No response received from target URL. Request details: ${JSON.stringify(error.config)}`);
            res.status(500).send('No response received from target URL. Check your internet connection or the URL.');
        } else {
            console.error(`[RENDER-DEBUG] An error occurred while setting up the request: ${error.message}`);
