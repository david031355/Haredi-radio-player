...
const cors = require('cors'); // נשאיר את cors למקרה שיהיו בעיות CORS עם תמונות/קובצי שמע חיצוניים, זה לא קשור לפרוקסי.
const app = express();
const PORT = process.env.PORT || 3000;

app.use(cors());

// --- נתיב יחיד לדף של תחנות הרדיו ---
app.get('/', (req, res) => {
    const cleanPageHtml = `
<!DOCTYPE html>
<html lang="he">
<head>
    ...
    <style>
        body {
            font-family: 'Noto Sans Hebrew', sans-serif; /* גופן עברי מגוגל */
            background-image: url('https://www.photo-art.co.il/wp-content/uploads/2015/09/BY1A4457.webp'); /* הוספת תמונה כרקע */
        ...
        .radio-item {
            ...
            width: 250px;
            text-align: center;
            transition: border 0.3s ease-in-out; /* מעבר חלק לשינוי הגבול */
        }
        .radio-item.active {
            border: 3px solid #ff9800; /* גבול כתום עבה לתחנה הפעילה */
        }
        .radio-item img {
            width: 150px;
            height: 150px;
...
<body>
    ...
    <script>
        let currentActiveItem = null; // משתנה לשמירת האלמנט הפעיל כרגע

        function playAudio(audioURL, clickedElement) {
            var audioPlayer = document.getElementById('audioPlayer');
            var audioSource = document.getElementById('audioSource');
            audioSource.src = audioURL;
            audioPlayer.load();  // טוען את קובץ השמע
            audioPlayer.play();  // משמיע את השמע

            // הסר את הקלאס 'active' מהפריט הפעיל הקודם (אם קיים)
            if (currentActiveItem) {
                currentActiveItem.classList.remove('active');
            }

            // הוסף את הקלאס 'active' לתחנה שנלחצה
            const radioItem = clickedElement.closest('.radio-item');
            radioItem.classList.add('active');
            currentActiveItem = radioItem; // שמור את הפריט החדש כפעיל
        }
    </script>
    ...
    res.send(cleanPageHtml);
});

// מפעיל את השרת
app.listen(PORT, () => {
    console.log(`Server listening on port ${PORT}`);
    console.log(`Visit http://localhost:${PORT} to see the radio page.`);
});